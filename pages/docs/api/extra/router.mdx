...
## `reload()` and `million:navigate` event
...
If you have client side scripts that run DOM modifications upon load, you can use the `reload()` function.

```js
import { reload } from 'million/router';
