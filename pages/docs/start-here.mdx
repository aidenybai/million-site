...
````js
import { m, render, schedule, style, Flags } from 'https://cdn.skypack.dev/million';
...
const search = (query = '') => {
  update(
    entries.map((entry) => {
      ...
      const shouldShow = compareTwoStrings(entry.toLowerCase(), query.trim().toLowerCase()) > 0.5;

      return m(
        'li',
        {
          key: `${entry}-${shouldShow}`,
          style: style({ color: shouldShow ? undefined : 'gray' }),
        },
        [entry],
      );
    }),
